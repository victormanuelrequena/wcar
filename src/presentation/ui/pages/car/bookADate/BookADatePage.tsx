...
import GetAvailableHoursForBuyUseCase, {
    GetAvailableHoursForBuyUseCaseName,
} from "../../../../../domain/use_cases/book/GetAvailableHoursForBuyUseCase";
...
import BookADateForBuyUseCase, {
    BookADateForBuyUseCaseName,
} from "../../../../../domain/use_cases/book/BookADateForBuyUseCase";
import GetAvailableDatesForBuyUseCase, {
    GetAvailableDatesForBuyUseCaseName,
} from "../../../../../domain/use_cases/book/GetAvailableDatesForBuyUseCase";
...
import GetAvailableDatesForSellUseCase, {
    GetAvailableDatesForSellUseCaseName,
} from "../../../../../domain/use_cases/book/GetAvailableDatesForSellUseCase";
import GetAvailableHoursForSellUseCase, {
    GetAvailableHoursForSellUseCaseName,
} from "../../../../../domain/use_cases/book/GetAvailableHoursForSellUseCase";
...
import BookADateForSeeUseCase, {
    BookADateForSeeUseCaseName,
} from "../../../../../domain/use_cases/book/BookADateForSeeUseCase";
...
const BookADatePage: FC<{}> = () => {
        ...
        try {
            if (action == BookADateActions.sell) {
                const response = await di
                    .get<GetAvailableDatesForSellUseCase>(GetAvailableDatesForSellUseCaseName)
                    .call(undefined);
                setAvailableDates(response);
            } else if (action == BookADateActions.book || action == BookADateActions.see) {
                const response = await di
                    .get<GetAvailableDatesForBuyUseCase>(GetAvailableDatesForBuyUseCaseName)
                    .call(carId!);
                setAvailableDates(response);
            } else {
                setAvailableDates([]);
    ...
    const _handleDateChange = async (bookDateId: string) => {
        try {
            ...
            if (action == BookADateActions.sell) {
                const response = await di
                    .get<GetAvailableHoursForSellUseCase>(GetAvailableHoursForSellUseCaseName)
                    .call(bookDateId, undefined);
                setAvailableHours(response);
            } else if (action == BookADateActions.book || action == BookADateActions.see) {
                const response = await di
                    .get<GetAvailableHoursForBuyUseCase>(GetAvailableHoursForBuyUseCaseName)
                    .call(bookDateId, carId!);
                setAvailableHours(response);
            } else {
                setAvailableHours([]);
            }
    ...
    const _handleConfirmBook = async (data: any) => {
        ...
        try {
            ...
            } else if (action == BookADateActions.book)
                await di
                    .get<BookADateForBuyUseCase>(BookADateForBuyUseCaseName)
                    .call(data.date, data.hour, carId!, data.contact, carId!);
            else if (action == BookADateActions.see)
                await di
    ...
    const _getCurrentStep = () => {
        ...
        else if (contact?.name == "" || contact?.lastname == "" || contact?.email == "" || contact?.phone == "")
            return 2;
    ...
    useEffect(() => {
        ...
        if (
            ((carId == null || carId.length <= 0) &&
                (action == BookADateActions.see || action == BookADateActions.book)) ||
            (buyData == null && action == BookADateActions.sell)
        )
            navigate(routes.error_404.relativePath);
        getAvailableDates();
                                ...
                                <section className="pb-4">
                                    ...
                                    {availableDates ? (
                                        <PickerBoxComponent
                                            ...
                                            options={availableDates.map((option) => {
                                                return {
                                        ...
                                        <>
                                            {availableHours && availableHours.length > 0 && (
                                                <PickerBoxComponent
                                                    formFunctions={formFunctions}
                                                    keyName="hour"
                                                    options={availableHours.map((option) => {
                                                        return {
                                                ...
                                                <div className="row">
                                                    ...
                                                    <div className="form-group col-md-6 my-3">
                                                        ...
                                                        <input
                                                            type="text"
                                                            className="form-control"
                                                            placeholder="número de celular o telefono"
                                                            {...register(
                                                                "contact.phone",
                                                                Validators({
                                                                    required: true,
                                ...
                                <section>
                                    <div className="form-check mb-4">
                                        <input
                                            ...
                                            type="checkbox"
                                            {...register("terms", Validators({ required: true }))}
                                        />
                                        <label className="form-check-label">
                                            Acepto tratamiento de datos personales.{" "}
                                            <Link to={"#"} className="text_underline text_gray">
                                                {" "}
                                    ...
                                    <button type="submit" className="btn btn_orange" disabled={_getCurrentStep() < 4}>
                                        CONFIRMAR CITA <Icons.CheckRounded />{" "}
                                    </button>
                                </section>
