...
const QuoteYourCarPage: FC<{}> = () => {
    ...
    const _handleSave = async (data: any) => {
        ...
        const dataToSend = {
            year: data?.car?.year,
            brand: data?.car?.brandId,
            version: data?.car?.version,
            model: data?.car?.model,
            city: data?.car?.cityId,
            mileage: data?.car?.kilometers,
            color: data?.car?.colorId,
    ...
    const _handleChangeModel = async (model: string) => {
        ...
        try {
            const response = await di
                .get<GetModelVersionByModelAndBrandIdUseCase>(GetModelVersionByModelAndBrandIdUseCaseName)
                .call(brandId, model);
                                                ...
                                                <div className="col-md-6 my-2">
                                                    <div className="form-group">
                                                        ...
                                                        >
                                                            ...
                                                            {Array.from(
                                                                Array(new Date().getFullYear() - _min_year + 1).keys()
                                                            ).map((year, index) => (
                                                                <option
                                                                    key={index}
